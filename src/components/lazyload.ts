export function install(Vue: typeof Vue, options) {
    ...
    var lazyLoadHandler = function(listener:any){
    ...
    var loadImageAsync = function (item:any, success:any, error:){
        var img = new Image()
        img.src = item.src
        img.onload = function(){
            success(item)
    ...
    var render = function render(item:any) {
        if (item.try >= init.try) {
            return false
        }
        item.try++;
        loadImageAsync(item, function(item:any){
    ...
    var getPosition = function getPosition(el:any) {
        var t = el.offsetTop
